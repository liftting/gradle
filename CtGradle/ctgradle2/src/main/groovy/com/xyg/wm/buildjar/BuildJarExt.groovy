...
class BuildJarExt  {
    ...
    public static BuildJarExt getConfig(Project project) {
        BuildJarExt config =
                project.getExtensions().findByType(BuildJarExt.class);
        if (config == null) {
            config = new BuildJarExt();
        }
        return config;
    }
